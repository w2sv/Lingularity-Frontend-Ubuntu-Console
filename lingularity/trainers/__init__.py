...
class Trainer(ABC):
    ...
    def __init__(self):
        ...
        self._language: str = self._select_language()  # equals reference language in case of english training
    ...
    # .Training History
    # -----------------
    def _load_training_history(self) -> Dict[str, Dict[str, int]]:
    ...
    def _append_2_training_history(self):
        training_history = self._load_training_history()
        trainer_abbreviation = self.__class__.__name__[0].lower()
        if training_history.get(self._today) is not None and training_history[self._today].get(trainer_abbreviation) is not None:
            training_history[self._today][trainer_abbreviation] += self._n_trained_items
