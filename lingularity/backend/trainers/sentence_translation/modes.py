...
class TrainingMode(ABC):
	...
	def _filter_sentence_data(
		...
		logging.info(f'Token occurrence median: {token_occurrence_median}')
...
class DictionExpansion(TrainingMode):
	@staticmethod
	def filter_sentence_data(sentence_data: SentenceData, language: str) -> SentenceData:
		return TrainingMode._filter_sentence_data(sentence_data, language, comparison_function=op.le)
